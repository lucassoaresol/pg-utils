...
var MigrationManager = class {
  ...
  async applyMigration(fileName, direction) {
    const filePath = (0, import_node_path2.join)(this.migrationsPath, fileName);
  ...
  async applyAllMigrations() {
    ...
    const allMigrations = await (0, import_promises2.readdir)(this.migrationsPath);
...
var migrationsDir = (0, import_node_path3.resolve)("migrations");
var configFilePath = (0, import_node_path3.resolve)("pg-utils.json");
var gitignorePath = (0, import_node_path3.resolve)(".gitignore");
async function loadClientsConfig() {
  try {
    const configFileContent = await (0, import_promises3.readFile)(configFilePath, "utf-8");
    const config = JSON.parse(configFileContent);
    const clientsMap = /* @__PURE__ */ new Map();
    config.forEach((client) => {
      const pgUtilsInstance = new src_default(
        client.user,
        client.host,
        client.password,
        client.port,
...
program.command("init").description(
  ...
  try {
    await (0, import_promises3.mkdir)(migrationsDir, { recursive: true });
    ...
    try {
      await (0, import_promises3.access)(configFilePath);
    ...
    try {
      await (0, import_promises3.access)(gitignorePath);
      const gitignoreContent = await (0, import_promises3.readFile)(gitignorePath, "utf-8");
      if (!gitignoreContent.includes("pg-utils.json")) {
        await (0, import_promises3.appendFile)(gitignorePath, "pg-utils.json\n");
        console.log(`Arquivo "${configFilePath}" adicionado ao .gitignore.`);
      } else {
        console.log(`"${configFilePath}" j\xE1 est\xE1 no .gitignore.`);
      }
    } catch {
      await (0, import_promises3.writeFile)(gitignorePath, "pg-utils.json\n", "utf-8");
...
program.command("add").description("Adiciona um novo cliente ao arquivo de configura\xE7\xE3o pg-utils.json").requiredOption("-i, --id <id>", "ID do cliente").requiredOption("-u, --user <user>", "Usu\xE1rio do banco de dados").requiredOption("-h, --host <host>", "Host do banco de dados").requiredOption("-p, --password <password>", "Senha do banco de dados").requiredOption("-P, --port <port>", "Porta do banco de dados", "5432").requiredOption("-d, --database <database>", "Nome do banco de dados").action(async (options) => {
  ...
  try {
    ...
    await (0, import_promises3.writeFile)(configFilePath, JSON.stringify(config, null, 2), "utf-8");
